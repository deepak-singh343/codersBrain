...
window.onload=function(){
    ...
    // an array of objects that define different rectangles
    var rects=[];
    rects.push({x:50,y:50,width:50,height:50,fill:"#444444",isDragging:false,previousRotation:0});
    rects.push({x:150,y:50,width:50,height:50,fill:"#ff550d",isDragging:false,previousRotation:0});
    rects.push({x:250,y:50,width:50,height:50,fill:"#800080",isDragging:false,previousRotation:0});
    rects.push({x:350,y:50,width:50,height:50,fill:"#0c64e8",isDragging:false,previousRotation:0});
   ...
   function rotateTopreviousState(){
        for(let rect of rects){
            ctx.save()
            ctx.rotate(0);
            ctx.clearRect(rect.x,rect.y,rect.width,rect.height);
            ctx.translate(rect.x + rect.width / 2, rect.y + rect.height / 2);
            ctx.rotate((Math.PI / 180) * (rect.previousRotation));
            ctx.translate(-(rect.x + rect.width / 2), -(rect.y + rect.height / 2));
    ...
    function rotate(selectedRect,direction){
        clearCanvas()
        let rect = rects[selectedRect]
        ctx.save()
        ctx.translate(rect.x + rect.width / 2, rect.y + rect.height / 2);
        rect.previousRotation+=50 * direction
        ctx.rotate((Math.PI / 180) * (rect.previousRotation));
        ctx.translate(-(rect.x + rect.width / 2), -(rect.y + rect.height / 2));
        ctx.fillStyle=rect.fill;
        shape(rect.x, rect.y, rect.width, rect.height, rect.fill);
        ctx.restore()
        // ctx.rotate((Math.PI / 180) * (currentAngle -= 50));
        rotateTopreviousState()
    }
